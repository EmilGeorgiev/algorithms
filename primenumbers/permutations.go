...
// PermutationsWithDuplicationCount calculates the number of permutations
// with duplication for a given set of n elements and a specified value of k.
// It uses the formula: P(n, k) = n^k
//
// Params:
// - n: the total number of elements in the set.
// - k: the number of elements to select from the input set for each permutation.
...
func PermutationsWithDuplicationCount(n, k int) int {
	return int(math.Pow(float64(n), float64(k)))
}

// PermutationsWithDuplication generates all the permutations with duplication
// for a given slice of elements and a specified value of k.
//
// The function uses a recursive approach:
// 1. Base case: If k == 0, return an empty permutation.
// 2. For each element in the input slice,
//   - generate all permutations of size (k-1) using the input slice (allowing duplication),
//   - append the current element to the beginning of each generated permutation,
//   - add the resulting permutation to the result set.
//
// Params:
...
//
// Returns:
//   - a 2D slice of T containing all permutations with duplication
//     of the input elements of size k.
func PermutationsWithDuplication[T any](elements []T, k int) [][]T {
	if k == 0 {
		return [][]T{{}}
	}
	...
	for _, element := range elements {
		for _, perm := range PermutationsWithDuplication(elements, k-1) {
			result = append(result, append([]T{element}, perm...))
		}
	}
